...
struct Cli {
    ...
    /// Poll interval in milliseconds when active
...
async fn main() {
    ...
    let token = authenticate(&http_client, &server_url, &cli.secret)
        .await
    ...
    let poll_idle = Duration::from_millis(cli.poll_idle_ms);
    let poll_active = Duration::from_millis(cli.poll_active_ms);
    ...
    info!("SOCKS5 proxy listening on {}", cli.listen);
